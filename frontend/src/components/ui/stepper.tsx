...
export default function Stepper({
  ...
  return (
    ...
    >
      <div
        className={`mx-auto w-full max-w-2xl rounded-2xl shadow-xl bg-card border border-border ${stepCircleContainerClassName}`}
      >
          ...
          <div className={`px-8 pb-8 ${footerClassName}`}>
            <div className={`mt-10 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              ...
              <button
                ...
                className="duration-350 flex items-center justify-center rounded-lg bg-primary py-2 px-6 font-medium tracking-tight text-primary-foreground transition hover:bg-primary/90 active:scale-95"
                {...nextButtonProps}
              >
...
export function Step({ children }: StepProps) {
  return <div className="px-8 w-full">{children}</div>;
}
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators = false }: StepIndicatorProps) {
  ...
  return (
    ...
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'hsl(var(--secondary))', color: 'hsl(var(--muted-foreground))' },
...
function StepConnector({ isComplete }: StepConnectorProps) {
  ...
  return (
    <div className="relative mx-3 h-1 flex-1 overflow-hidden rounded bg-secondary">
      <motion.div
