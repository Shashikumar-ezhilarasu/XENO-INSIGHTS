...
with open('frontend/src/app/(app)/campaigns/page.tsx', 'w') as f:
    f.write(campaigns_content)


# Now modify segments/page.tsx to parse query params
segments_path = 'frontend/src/app/(app)/segments/page.tsx'
with open(segments_path, 'r') as f:
    segments_content = f.read()

# Replace next/navigation imports to include useSearchParams
segments_content = segments_content.replace(
    'import { useRouter } from "next/navigation";',
    'import { useRouter, useSearchParams } from "next/navigation";\nimport { Suspense } from "react";'
)

# Insert a wrapper component to use Suspense
main_export_start = segments_content.find("export default function AISegmentsStudio()")
wrapper_code = """
...
# Replace the component close and add the Suspense wrapper at the bottom
wrapper_close = """
export default function AISegmentsStudio() {
  return (
...
# Append the wrapper to the very end of the file
segments_content = segments_content + wrapper_close

# Now inject the useEffect for query parsing after state declarations
# We need to find a good spot after states.
# We'll look for `const [isGenerating, setIsGenerating] = useState(false);`
hook_injection = """  const [isGenerating, setIsGenerating] = useState(false);

  // Hook for AI Template Navigation
  useEffect(() => {
    if (template === 'artisan_roast') {
      setQuery("Act as a luxury brand marketer. Design a VIP Rich Media campaign to re-engage high-spenders, similar to the Artisan Roast format.");
    } else if (template === 'neon_pulse') {
      setQuery("Act as a high-energy streetwear marketer. Design a vibrant social drop campaign for the Neon Pulse Sneaker Launch targeting apparel enthusiasts.");
    } else if (template === 'email_newsletter') {
      setQuery("Act as a retention specialist. Draft a comprehensive long-form email newsletter summarizing this month's updates for our active subscribers.");
    } else if (template === 'direct_chat') {
      setQuery("Act as a performance marketer. Draft a high-conversion, low-latency SMS blast for an upcoming 24-hour flash sale.");
    }
  }, [template]);
"""
